...
int main(int argc, char *argv[]) {
    ...
    for (int k = 0; k < NTIMES + 1; k++) {
        ...
        start = omp_get_wtime();
        #pragma omp parallel for default(none) shared(b, c, N) firstprivate(scalar) schedule(static)
        for (size_t i = 0; i < N; i++) {
            b[i] = scalar * c[i];
        }
        end = omp_get_wtime();
        ...
        start = omp_get_wtime();
        #pragma omp parallel for default(none) shared(a, b, c, N) firstprivate(scalar) schedule(static)
        for (size_t i = 0; i < N; i++) {
            a[i] = b[i] + scalar * c[i];
        }
        end = omp_get_wtime();
